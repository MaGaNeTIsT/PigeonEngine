...
#include "../../../Sources/EngineCore/Base/Headers/CEBaseType.h"
...
#define CUSTOM_IS_QNAN(f)	(f != f)
	...
	Quaternion _GTranslateQuaternion(const aiQuaternion& v)
	{
		return (Quaternion(v.x, v.y, v.z, v.w));
	}

	Vector3 _GTranslateVector3(const aiVector3D& v)
	{
		return (Vector3(v.x, v.y, v.z));
	}

	void _GTranslateMatrix(const aiMatrix4x4& m, Vector3& pos, Quaternion& rot, Vector3& scl)
	{
	...
	Matrix4x4 _GTranslateMatrix(const aiMatrix4x4& m, const BOOL& useOrigin = TRUE)
	{
		if (useOrigin)
		{
