...
#define CUSTOM_IS_QNAN(f)	(f != f)
	...
	static void GatherBoneWorldTransformsRecursion(const aiNode* InNode, const Matrix4x4& InParentWorldTransform, TMap<EString, Matrix4x4>& OutBoneWorldTransforms)
	{
		if (InNode == nullptr)
		{
			return;
		}
		Matrix4x4 LocalTransform = AssimpTranslateMatrix(InNode->mTransformation, TRUE);
		Matrix4x4 WorldTransform = Matrix4x4::MultiplyMatrix(InParentWorldTransform, LocalTransform);
		EString NodeName = AssimpTranslateString(InNode->mName);
		if (!(OutBoneWorldTransforms.ContainsKey(NodeName)))
		{
			OutBoneWorldTransforms.Add(NodeName, WorldTransform);
		}
		for (UINT32 ChildIndex = 0u; ChildIndex < InNode->mNumChildren; ChildIndex++)
		{
			GatherBoneWorldTransformsRecursion(InNode->mChildren[ChildIndex], WorldTransform, OutBoneWorldTransforms);
		}
	}
	static void GatherSkinnedMeshBounds(ESkinnedMesh& InOutMesh, const aiScene* InScene)
	{
		// Build bone world transform map from scene node hierarchy
		TMap<EString, Matrix4x4> BoneWorldTransforms;
		GatherBoneWorldTransformsRecursion(InScene->mRootNode, Matrix4x4::Identity(), BoneWorldTransforms);

		// Build reverse index map: USHORT index -> bone name
		const ESkinnedMesh::EBindPoseIndex& BindPoseIndex = InOutMesh.GetBindPoseIndex();
		const ESkinnedMesh::EBindPoseValue& BindPoseValue = InOutMesh.GetBindPoseValue();
		TMap<USHORT, EString> IndexToBoneName;
		for (auto It = BindPoseIndex.Begin(); It != BindPoseIndex.End(); It++)
		{
			IndexToBoneName.Add(It->second, It->first);
		}
		...
		if (!(InOutMesh.GetVertexElement(EVertexLayoutType::MESH_VERTEX, 0u, VertexDataPtr)) || !VertexDataPtr)
		{
			InOutMesh.SetBoundAABB(Vector3::Zero(), Vector3::Zero());
			return;
		}
		const UINT32 VertexElementNum = VertexDataPtr->Stride / sizeof(FLOAT);
		const UINT32 VertexCount = VertexDataPtr->ElementNum;

		// Get skin data
		const ESkinData* SkinDataPtr = nullptr;
		InOutMesh.GetSkinElement(0u, SkinDataPtr);

		for (UINT32 VertexIndex = 0u; VertexIndex < VertexCount; VertexIndex++)
		{
			Vector3 RawPos(Vector3::Zero());
			if (VertexElementNum > 0u) { RawPos.x = VertexDataPtr->Datas[VertexIndex * VertexElementNum + 0u]; }
			if (VertexElementNum > 1u) { RawPos.y = VertexDataPtr->Datas[VertexIndex * VertexElementNum + 1u]; }
			if (VertexElementNum > 2u) { RawPos.z = VertexDataPtr->Datas[VertexIndex * VertexElementNum + 2u]; }

			Vector3 SkinnedPos(Vector3::Zero());
			FLOAT TotalWeight = 0.f;

			if (SkinDataPtr && SkinDataPtr->Indices && SkinDataPtr->Weights)
			{
				const UINT32 SkinStrideIn32Bits = SkinDataPtr->Stride / sizeof(FLOAT);
				for (UINT32 WeightSlot = 0u; WeightSlot < SkinStrideIn32Bits; WeightSlot++)
				{
					FLOAT Weight = SkinDataPtr->Weights[VertexIndex * SkinStrideIn32Bits + WeightSlot];
					if (Weight <= 0.f)
					{
						continue;
					}
					USHORT BoneIdx = static_cast<USHORT>(SkinDataPtr->Indices[VertexIndex * SkinStrideIn32Bits + WeightSlot]);
					EString BoneName;
					if (!(IndexToBoneName.FindValue(BoneIdx, BoneName)))
					{
						continue;
					}
					const Matrix4x4* OffsetMatrixPtr = BindPoseValue.FindValueAsPtr(BoneName);
					const Matrix4x4* WorldTransformPtr = BoneWorldTransforms.FindValueAsPtr(BoneName);
					if (OffsetMatrixPtr && WorldTransformPtr)
					{
						Matrix4x4 SkinMatrix = Matrix4x4::MultiplyMatrix(*WorldTransformPtr, *OffsetMatrixPtr);
						Vector3 TransformedPos = Matrix4x4TransformPosition(SkinMatrix, RawPos);
			...
			// Normalize by total weight to avoid pulling toward origin when weights don't sum to 1
			// Fall back to raw position if no valid skin weights were found
			Vector3 FinalPos = (TotalWeight > 0.f) ? (SkinnedPos / TotalWeight) : RawPos;
			ResultBound.AABBMin = MinVector3(FinalPos, ResultBound.AABBMin);
			ResultBound.AABBMax = MaxVector3(FinalPos, ResultBound.AABBMax);
		}
	...
	CAssimpManager::CReadFileStateType CAssimpManager::ReadSkinnedMeshFile(const EString& InPath, TArray<ESkinnedMesh>& OutMeshes, const BOOL32 IsCombineSubmeshes)
	{
		...
		if (IsCombineSubmeshes)
		{
			...
			GatherSkinnedMeshBounds(NewSkinnedMesh, Scene);
		}
		else
		{
			TranslateAssimpMeshToEngineMeshInternal(EngineLayouts, EngineLayoutNum, Meshes, OutMeshes);
			...
			for (INT32 OutMeshIndex = 0, OutMeshNum = OutMeshes.Num(); OutMeshIndex < OutMeshNum; OutMeshIndex++)
			{
				GatherSkinnedMeshBounds(OutMeshes[OutMeshIndex], Scene);
			}
		}
