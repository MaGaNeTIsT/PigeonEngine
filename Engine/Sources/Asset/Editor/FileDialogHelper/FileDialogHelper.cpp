...
#define FILE_DIALOG_HELPER_MAX_STRLEN 256
    ...
    BOOL32 OpenDialog(const CHAR* filterList, const CHAR* defaultPath, CHAR** outPath)
    {
#if _WINDOWS
    ...
    BOOL32 OpenDialogMultiSelect(const CHAR* filterList, const CHAR* defaultPath, PathSet* outPaths)
    {
#if _WINDOWS
...
#if _WINDOWS
    static BOOL32 CopyCharToWChar(const CHAR* inStr, WCHAR** outStr)
    {
        INT32 inStrByteCount = static_cast<INT32>(::strlen(inStr));
        INT32 charsNeeded = ::MultiByteToWideChar(CP_UTF8, 0,
        ...
        *outStr = (WCHAR*)::malloc(charsNeeded * sizeof(WCHAR));
        ...
        INT32 ret = ::MultiByteToWideChar(CP_UTF8, 0,
            inStr, inStrByteCount,
            *outStr, charsNeeded);
        (*outStr)[charsNeeded - 1] = '\0';

        INT32 inStrCharacterCount = static_cast<INT32>(::strlen(inStr));
    ...
    {
        INT32 inStrCharacterCount = static_cast<INT32>(::wcslen(inStr));
        INT32 bytesNeeded = ::WideCharToMultiByte(CP_UTF8, 0,
        ...
        *outStr = (CHAR*)::malloc(bytesNeeded);
        ...
        INT32 bytesWritten = ::WideCharToMultiByte(CP_UTF8, 0,
            inStr, -1,
            *outStr, bytesNeeded,
            NULL, NULL);
        assert(bytesWritten);
    ...
    static BOOL32 SetDefaultPath(IFileDialog* dialog, const CHAR* defaultPath)
    {
    ...
    static BOOL32 AddFiltersToDialog(::IFileDialog* fileOpenDialog, const CHAR* filterList)
    {
        ...
        // Count rows to alloc
        UINT32 filterCount = 1; /* guaranteed to have one filter on a correct, non-empty parse */
        ...
        for (UINT32 i = 0; i < filterCount + 1; ++i)
        {
        ...
        while (TRUE)
        {
            if ((*p_filterList) == ',' || (*p_filterList) == ';' || (*p_filterList) == '\0')
            {
                ...
                INT32 bytesWritten = ::sprintf_s(extWildcard, FILE_DIALOG_HELPER_MAX_STRLEN, "*.%s", typebuf);
                assert(bytesWritten == (INT32)(::strlen(typebuf) + 2));
            ...
            BOOL32 tempbool = (*p_filterList) == ',' || (*p_filterList) == ';' || (*p_filterList) == '\0';
            if (!tempbool)
            {
...
#pragma warning(once : 6001)
    ...
    static INT32 CopyWCharToExistingCharBuffer(const WCHAR* inStr, CHAR* outPtr)
    {
        INT32 bytesNeeded = static_cast<INT32>(GetUTF8ByteCountForWChar(inStr));
    ...
    static BOOL32 AllocPathSet(IShellItemArray* shellItems, PathSet* pathSet)
    {
...
#pragma warning(once : 6387)
        ...
        for (DWORD i = 0; i < numShellItems; ++i)
        {
            ...
            INT32 bytesWritten = CopyWCharToExistingCharBuffer(name, p_buf);
            ::CoTaskMemFree(name);
    ...
    BOOL32 OpenDialogWindows(const CHAR* filterList, const CHAR* defaultPath, CHAR** outPath)
    {
        BOOL32 RESULT = FALSE;
    ...
    BOOL32 OpenDialogMultiSelectWindows(const CHAR* filterList, const CHAR* defaultPath, PathSet* outPaths)
    {
        BOOL32 RESULT = FALSE;
