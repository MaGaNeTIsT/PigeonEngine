#ifndef _ENGINE_CLOTH_ANISOTROPIC_PS_HLSL
#define _ENGINE_CLOTH_ANISOTROPIC_PS_HLSL
...
Texture2D	_AnisotropicTexture				: register(t12);
Texture2D	_AnisotropicDirectionTexture	: register(t13);
...
float4 main(VaryingForward input) : SV_Target
{
	...
	float    materialAnisotropy		= _AnisotropicTexture.Sample(_LinearWrapSampler, input.uv0).r * _AnisotropyStrengthDirection.x;
	float    materialAnisotropyDir	= _AnisotropicDirectionTexture.Sample(_LinearWrapSampler, input.uv0).r * _AnisotropyStrengthDirection.y;
	...
	pixelParams.F0								= ComputeFresnelF0(0.16, GBuffer.BaseColor, 0.0);
#ifdef MATERIAL_HAS_SHEEN_COLOR
	...
	for (uint i = 0u; i < (uint)_LightCount.x; i++)
	{
		...
		color.rgb += SurfaceShading_ClothAnisotropic(
			pixelParams,
			content,
			light,
			viewDirWS,
