...
UINT32 CDebugScreen::DEBUGPOLYGON_COUNT = 0u;
...
{
	INT32 length = pipelineSize.Y() / CDebugScreen::DEBUGPOLYGON_COUNT;
	INT32 polygonCount = static_cast<INT32>(CDebugScreen::DEBUGPOLYGON_COUNT);
...
{
	UINT32 size = static_cast<UINT32>(m_Polygons.size());
	if (size > 0u)
	{
...
void CDebugScreen::PrepareDraw()
{
	...
	for (INT32 i = 0; i < static_cast<INT32>(CManager::GetRenderPipeline()->GEOMETRY_BUFFER_COUNT); i++)
	{
...
{
	this->PrepareDraw();
	for (UINT32 i = 0; i < CDebugScreen::DEBUGPOLYGON_COUNT; i++)
	{
