#ifndef _ENGINE_DEFAULT_FORWARD_PS_HLSL
#define _ENGINE_DEFAULT_FORWARD_PS_HLSL
...
Texture2D	_AnisotropyTexture			: register(t11);
Texture2D	_AnisotropyDirectionTexture	: register(t12);
Texture2D	_ClearCoatStrengthTexture	: register(t13);
Texture2D	_ClearCoatRoughnessTexture	: register(t14);
Texture2D	_ClearCoatNormalTexture		: register(t15);

cbuffer ConstantBufferMaterialData : register(b3)
{
	float4	_BaseColorRoughness;
	float4	_EmissiveAmbientOcclusion;
	float4	_MetallicnessReflectanceAnisotropy;
	float4	_ClearCoatStrengthRoughness;
}
...
float4 main(VaryingForward input) : SV_Target
{
	...
	float    materialMetallicness	= _MetallicnessTexture.Sample(_LinearWrapSampler, input.uv0).r * _MetallicnessReflectanceAnisotropy.x;
	float    materialReflectance	= ComputeFresnelF0Dielectric(_MetallicnessReflectanceAnisotropy.y);
#ifdef MATERIAL_HAS_ANISOTROPY
	float    materialAnisotropy		= _AnisotropyTexture.Sample(_LinearWrapSampler, input.uv0).r * _MetallicnessReflectanceAnisotropy.z;
	float3   materialAnisotropyDir	= TransformTangentToSpaceDir(SafeNormalize(_AnisotropyDirectionTexture.Sample(_LinearWrapSampler, input.uv0).rgb * 2.0 - 1.0), tangentToWorld);
#endif
#ifdef MATERIAL_HAS_CLEAR_COAT
	float    materialClearCoatStr	= _ClearCoatStrengthTexture.Sample(_LinearWrapSampler, input.uv0).r * _ClearCoatStrengthRoughness.x;
	float    materialClearCoatRou	= _ClearCoatRoughnessTexture.Sample(_LinearWrapSampler, input.uv0).r * _ClearCoatStrengthRoughness.y;
#ifdef MATERIAL_HAS_CLEAR_COAT_NORMAL
	float3   materialClearCoatNor	= TransformTangentToSpaceDir(SafeNormalize(_ClearCoatNormalTexture.Sample(_LinearWrapSampler, input.uv0).rgb * 2.0 - 1.0), tangentToWorld);
#endif
#endif
...
#endif
#ifdef MATERIAL_HAS_CLEAR_COAT
	pixelParams.ClearCoatStrength				= materialClearCoatStr;
	pixelParams.ClearCoatPerceptualRoughness	= clamp(materialClearCoatRou, MIN_PERCEPTUAL_ROUGHNESS, 1.0);
	pixelParams.ClearCoatRoughness				= ComputePerceptualRoughnessToRoughness(pixelParams.ClearCoatPerceptualRoughness);
#ifdef MATERIAL_HAS_CLEAR_COAT_NORMAL
	pixelParams.ClearCoatNormal					= materialClearCoatNor;
#endif
#endif
