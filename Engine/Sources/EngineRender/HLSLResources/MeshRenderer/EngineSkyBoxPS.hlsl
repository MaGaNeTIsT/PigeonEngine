...
cbuffer ConstantBufferSkyBox : register(b1)
{
	float4 _Parameter;	//x y z is origin position of sky sphere. w is radius of sky sphere.
};
...
{
	const float3 skyOrigin = _Parameter.xyz;
	const float skyRadius = _Parameter.w;
