...
cbuffer ConstantBufferSWECommon : register(b1)
{
	float SWE_InvResX;
	float SWE_InvResY;
	float SWE_ResX;
	...
	float SWE_DeltaTime;
	float SWE_CaptureHeight;
	float Padding0;
	float Padding1;
	
	float4 UniversalVectorParams[8];
	int4 UniversalIntVectorParams[2];
	uint4 UniversalUintVectorParams[2];
};
