...
Texture2D		VelocityHeightFoamTex		: register(t0);
Texture2D		GroundTex					: register(t1);
RWTexture2D		OutIntegrateHeight			: register(u0);

#define SWE_USE_FLUX_SAMPLE_FUNCTION	1
#include "../ShallowWaterEquationCommon.ush"

#define FoamFadeSpeed					UniversalVectorParams[0].x
#define FoamShallowDepth				UniversalVectorParams[0].y
#define FoamShallowGenerate				UniversalVectorParams[0].z
#define FoamWaveGenerate				UniversalVectorParams[0].w
#define FluidTransportMax				UniversalVectorParams[1].x
#define AdvectFoam						UniversalVectorParams[1].y
#define EvaporationHeight				UniversalVectorParams[1].z
#define FluxEnergy						UniversalVectorParams[1].w
#define PixelAnchor						UniversalVectorParams[2].xy
#define PixelSize						UniversalVectorParams[2].zw
...
bool ComputeIntegrateHeight( in int2 Coord, out float4 OutValue )
{
	...
	[branch] if (any(Coord >= Res) || any(LocalCoord >= LocalRes))
	{
